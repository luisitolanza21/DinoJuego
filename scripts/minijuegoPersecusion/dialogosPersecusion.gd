...
var dialog = ["PERSIGUE AL AUTOMÓVIL SOSPECHOSO!!!"]
var dialog_index = 0
var finish = false
var text_speed = 0.1
var is_typing = false
...
func load_dialog():
	if dialog_index < dialog.size():
		...
		$labelTexto.bbcode_text = dialog[dialog_index]
		$labelTexto.visible_characters = 0
	
		var tween_duration = text_speed * dialog[dialog_index].length()
		current_tween = get_tree().create_tween()
		is_typing = true
		current_tween.tween_property($labelTexto, "visible_characters", dialog[dialog_index].length(), tween_duration)
